...
#NOTE(Jese): The premise is to generate 100 cutouts of size 1024x1024 pixels in parallel
# and then reject overlapping ones.
#
# The process repeats until at least 25 non-overlapping cutouts are produced, which is the expected first result.
#
...
if __name__ == "__main__":
    ...
    def main():
        ...
        while True:
            bounds_xy = randint(0, extent_xy, (100, 2))
        ...
        with rasterio.Env(GDAL_DISABLE_READDIR_ON_OPEN=True, GDAL_NUM_THREADS="ALL_CPUS", NUM_THREADS="ALL_CPUS"):
            with rasterio.open(mosaic_fp, 'r') as ds: #TODO(Jesse): Multiprocessing if this is slow somehow.
                for i, bound_xy in enumerate(bounds_xy):
                    ...
                    window = rasterio.windows.Window(bound_xy[0], bound_xy[1],
                                                     cutout_xy, cutout_xy)

                    pan_ndvi_img = ds.read(window=window)

                    window_transform = rasterio.windows.transform(window, ds.transform)
                    with rasterio.open(output_tif_path, 'w',
                            driver='GTiff', width=cutout_xy, height=cutout_xy, count=2,
                            dtype=pan_ndvi_img.dtype, crs=ds.crs, transform=window_transform,
                            compress='ZSTD', tiled=True, predictor=2, interleave="BAND") as new_data_set:
                        new_data_set.write(pan_ndvi_img)
